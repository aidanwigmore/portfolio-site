import Transcriptions from './Transcriptions';

export default [
  {
    title: 'Batched Support',
    description:
      'At MRKTBox I was tasked with publishing short videos demonstrating how to use the MRKTBox platform to complete common tasks. These videos were used as part of a larger initiative to provide more self-service support options for our customers, and to reduce the number of support tickets we received for common issues.',
    channel: 'Batched Support',
    src: [
      'https://www.youtube.com/embed/IJFOHNK-sBY?si=PXlTKlbJS7qaHsPR',
      'https://www.youtube.com/embed/eX7XDNOrv90?si=qR4uxR5TTRUHuZ-B',
      'https://www.youtube.com/embed/lQwYJoPhWwQ?si=WmTaPeI0Ib-7s82c',
    ],
    steps: {
      titles: ['Order Catering Items', 'Subscribe To MRKTBox', 'Sign up to MRKTBox'],
      descriptions: [],
      ratingCodes: ['batched-support-1', 'batched-support-2', 'batched-support-3'],
      uploadDates: ['2 Oct 2024', '2 Oct 2024', '2 Oct 2024'],
    },
    stepsLabels: Transcriptions[0].transcriptions,
  },
  {
    title: 'Capstone Project',
    description: 'TeaBank Demonstrations',
    channel: 'aidanOdyssey',
    src: [
      'https://www.youtube.com/embed/aFiUmvcF3Xw?si=R-781N2WEAGsqnZF',
      'https://www.youtube.com/embed/1SbgMCq4-PM?si=yABv-EKPftpFo7fv',
      'https://www.youtube.com/embed/fVJ_0QYCSoI?si=p7jX3Wn7ylXdaZxE',
    ],
    steps: {
      titles: ['Capstone Final Report', 'Capstone Report 3', 'Capstone Report 4'],
      descriptions: ['', '', ''],
      ratingCodes: ['capstone-1', 'capstone-2', 'capstone-3'],
      uploadDates: ['13 Dec 2024', '18 Nov 2024', '9 Dec 2024'],
    },
    stepsLabels: Transcriptions[1].transcriptions,
  },
  {
    title: 'PHP / Laravel',
    description: 'PHP Course Demonstrations',
    channel: 'aidanOdyssey',
    src: [
      'https://www.youtube.com/embed/enO66A4MEjA?si=hljKuYuVjIETDRgi',
      'https://www.youtube.com/embed/otVmSjKBpyQ?si=2JoxDWcCQPJFrSVl',
      'https://www.youtube.com/embed/XFhhSQ-mEgM?si=-UdqBiqhLYOWUhhK',
    ],
    steps: {
      titles: ['Assignment 5', 'Assignment 6', 'Assignment 7'],
      descriptions: [
        'Laravel, Restful API access database 7 different routes, get, post, put, 3 route params with filters, patients database\n\nDemonstrating postman to confirm functionality of API routes',
        '',
        '',
      ],
      ratingCodes: ['php-1', 'php-2', 'php-3'],
      uploadDates: ['25 Mar 2023', '7 Apr 2023', '15 Apr 2023'],
    },
    stepsLabels: Transcriptions[2].transcriptions,
  },
  {
    title: 'Soft Skills',
    description: 'Technical Presentations',
    channel: 'aidanOdyssey',
    src: [
      'https://www.youtube.com/embed/gZUH_6LCkm8?si=3t5TdT_27pHiLuFC',
      'https://www.youtube.com/embed/ZinB2DRHXHg?si=2dpYRMK34Yukf63_',
      'https://www.youtube.com/embed/8etYVCmutxc?si=i9NOLv4AkUvkNQDg',
    ],
    steps: {
      titles: [
        "Entrepreneurship in today's world - Video Pitch",
        'Tech Writing Presentation - Product Evaluation and Recommendation',
        'Tech Writing Presentation 3 (Job posting)',
      ],
      descriptions: ['', '', ''],
      ratingCodes: ['soft-skills-1', 'soft-skills-2', 'soft-skills-3'],
      uploadDates: ['7 Dec 2022', '1 Apr 2021', '30 Mar 2021'],
    },
    stepsLabels: Transcriptions[3].transcriptions,
  },
];
